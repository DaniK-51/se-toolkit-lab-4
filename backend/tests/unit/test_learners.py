...
def test_read_learners_filters_by_enrolled_after_boundary() -> None:
    ...
    mock_result.all = MagicMock(return_value=[
        Learner(id=3, name="Charlie", email="charlie @test.com", enrolled_at=datetime(2024, 6, 16, 8, 0, 0)),
    ])
    ...
    async def run_test():
        ...
        # Verify the filter was applied - only Charlie (enrolled after boundary) is returned
        assert len(result) == 1
...
def test_read_learners_with_none_returns_all() -> None:
    ...
    async def run_test():
        ...
        # All learners returned when no filter
        assert len(result) == 3
