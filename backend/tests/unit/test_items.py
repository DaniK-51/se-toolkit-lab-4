...
def test_put_item_returns_404_for_nonexistent_id() -> None:
    ...
    body = ItemUpdate(title="Updated Title", description="New description")
...
def test_put_item_updates_only_title_and_description() -> None:
    ...
    body = ItemUpdate(title="New Title", description="")
    
    async def run_test():
        result = await put_item(item_id=42, body=body, session=mock_session)
        assert result.title == "New Title"
        assert result.description == ""
        assert result.type == "task"  # Unchanged
        assert result.parent_id == 10  # Unchanged
